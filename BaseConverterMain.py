...
def baseConverterIntentHandler(intent):
    ...
    numToConvert = intent["toConvert"]["value"]
    initBase = int(intent["initBase"]["value"])
    finalBase = int(intent["finalBase"]["value"])

    convertedNum = int(str(int(numToConvert, initBase)), finalBase)


    session_attributes = ""
    title = "Base Converted"
    output = numToConvert + " in base " + finalBase + " is " + convertedNum
    reprompt_text = ""
    return build_response(session_attributes, 
    build_speechlet_response(title, output, reprompt_text, True))
...
def on_intent(intent_request, session):
    ...
    if intent_name == "BASECONVERTERINTENT":
        return baseConverterIntentHandler(intent["slots"])
    else:
